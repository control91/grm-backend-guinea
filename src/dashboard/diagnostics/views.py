...
class GetPrefectureByRegionId(AJAXRequestMixin, LoginRequiredMixin, JSONResponseMixin, generic.View):
    def get(self, request, *args, **kwargs):
        ...
        if region_id:
            selector["region_id"] = region_id

        issues = grm_db.get_query_result(selector)

        issues = [doc for doc in issues]

        return self.render_to_json_response(issues, safe=False)
